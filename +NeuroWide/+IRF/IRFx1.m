...
function [r, IRF, outParams, r_dt] = ...
    IRFx1(HbT, Ca, win, brain_mask, varargin)
...
IRF = NeuroWide.IRF.alpha_IRF(params(1), params(2), params(3), ...
    params(4), params(5), win);

if p.Results.norm
    conv = NeuroWide.IRF.convolve3d(Ca ./ std(Ca, 0, 3), IRF, win, ones(size(brain_mask)));
else
    conv = NeuroWide.IRF.convolve3d(Ca, IRF, win, ones(size(brain_mask)));
end
r = NeuroWide.connectivity.corr(HbT, conv, 3);

if ~isempty(p.Results.corrWin)
    r_dt = NeuroWide.IRF.hemCorrGram(HbT, conv, p.Results.corrWin);
end

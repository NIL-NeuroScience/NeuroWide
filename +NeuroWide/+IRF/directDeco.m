...
function [r, IRF] = directDeco(Y, X, win, brain_mask, varargin)
...
predictor_mat = NeuroWide.IRF.convmtx2d(dspredictor, l_irf);
...
pred_sig = NeuroWide.IRF.convolve3d(X, IRF, win, ones(size(brain_mask)));
r = NeuroWide.connectivity.corr(Y, pred_sig, 3);

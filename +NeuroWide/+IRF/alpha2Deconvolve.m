...
function [r, IRF, outParams, conv_Ca, conv_NE] = ...
    alpha2Deconvolve(HbT, Ca, NE, win, brain_mask, varargin)
...
dsNE = NeuroWide.process.downsample2d(NE, p.Results.ds);
...
if string(p.Results.method) == "direct"
    ...
    Ca_mat = NeuroWide.IRF.convmtx2d(dsCa, l_irf);
    Ca_mat = Ca_mat(1 - win(1) : end - win(2), :, :);
    Ca_mat = permute(Ca_mat, [2, 3, 1]); % n_IRF x N x T

    NE_mat = NeuroWide.IRF.convmtx2d(dsNE, l_irf);
    ...
    hrf1 = NeuroWide.IRF.alpha_IRF(t1, tau1, 0.5, A, 0, win);
    hrf2 = NeuroWide.IRF.alpha_IRF(t2, tau2, 0.5, B, 0, win);
    
    conv_Ca = sum(Ca_mat .* hrf1, 1);
    conv_NE = sum(NE_mat .* hrf2, 1);
    
    LR = NeuroWide.IRF.hemRegress(design_HbT, cat(4, conv_Ca, conv_NE), ones(1, N));
    ...
    LR = NeuroWide.IRF.hemRegress(permute(dsHbT, [3, 2, 1]), ...
...
hrf1 = NeuroWide.IRF.alpha_IRF(params(1), params(3), 0.5, 1, 0, win);
hrf2 = NeuroWide.IRF.alpha_IRF(params(2), params(4), 0.5, 1, 0, win);

conv_Ca = NeuroWide.IRF.convolve3d(Ca ./ std(Ca, 0, 3), hrf1, win, brain_mask);
conv_NE = NeuroWide.IRF.convolve3d(NE ./ std(NE, 0, 3), hrf2, win, brain_mask);

LR = NeuroWide.IRF.hemRegress(HbT ./ std(HbT, 0, 3), ...
...
r = NeuroWide.connectivity.corr(HbT ./ std(HbT, 0, 3), pred_HbT, 3);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%% EXTRA FUNCTIONS %%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% cost function for IRF optimization using direct method (MSE)
function J = f_hrf_cost_func(t1, t2, tau1, tau2, A, B, range, X_mat, y)
    HRF1 = NeuroWide.IRF.alpha_IRF(t1, tau1, 0.5, A, 0, range);
    HRF2 = NeuroWide.IRF.alpha_IRF(t2, tau2, 0.5, B, 0, range);

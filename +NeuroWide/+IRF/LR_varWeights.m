...
function [r, outParams, conv_Ca, conv_NE] = ...
    LR_varWeights(HbT, Ca, NE, win, brain_mask, varargin)
...
if ~isempty(p.Results.filter)
    Ca = NeuroWide.process.bpf(Ca, p.Results.filter, 10, 3);
    NE = NeuroWide.process.bpf(NE, p.Results.filter, 10, 3);
end
...
if p.Results.norm
    ...
    conv_Ca = NeuroWide.IRF.convolve3d(Ca, hrf1, win, brain_mask);
    conv_NE = NeuroWide.IRF.convolve3d(NE, hrf2, win, brain_mask);
    LR = NeuroWide.IRF.hemRegress(HbT, cat(4, conv_Ca, conv_NE), brain_mask);
end

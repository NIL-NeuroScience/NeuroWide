...
function [r, IRF, outParams, r_dt, pred_HbT] = ...
    IRFx1_varWeights(HbT, Ca, win, brain_mask, varargin)
...
if string(p.Results.method) == "direct"
    ...
    Ca_mat = NeuroWide.IRF.convmtx2d(dsCa, n_IRF);
    ...
    hrf1 = NeuroWide.IRF.alpha_IRF(t0, tau1, tau2, A, 0, win);
    hrf2 = NeuroWide.IRF.alpha_IRF(t0, tau1, tau2, 0, B, win);
    
    convPos = sum(Ca_mat .* hrf1, 1);
    convNeg = sum(Ca_mat .* hrf2, 1);
    
    LR = NeuroWide.IRF.hemRegress(design_HbT, cat(4, convPos, convNeg), ones(1, N));
...
% pixel-wise LR
hrf1 = NeuroWide.IRF.alpha_IRF(params(1), params(2), params(3), 1, 0, win);
hrf2 = NeuroWide.IRF.alpha_IRF(params(1), params(2), params(3), 0, -1, win);
...
convPos = NeuroWide.IRF.convolve3d(normCa, hrf1, win, brain_mask);
convNeg = NeuroWide.IRF.convolve3d(normCa, hrf2, win, brain_mask);

LR = NeuroWide.IRF.hemRegress(normHbT, cat(4, convPos, convNeg), brain_mask);
...
IRF = NeuroWide.IRF.alpha_IRF(params(1), params(2), params(3), outParams.A(:)', ...
    outParams.B(:)', win)';
...
r = NeuroWide.connectivity.corr(HbT, pred_HbT, 3);

if ~isempty(p.Results.corrWin)
    r_dt = NeuroWide.IRF.hemCorrGram(HbT, pred_HbT, p.Results.corrWin);
end
...
function J = f_hrf_cost_func(t0, tau1, tau2, A, B, range, X_mat, y)
    hrf = NeuroWide.IRF.alpha_IRF(t0, tau1, tau2, A, B, range);

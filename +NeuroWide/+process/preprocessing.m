...
function preprocessing(Mouse, Date, varargin)
%% parse inputs
p = inputParser;
addParameter(p,'runs',[]);
...
for rootFolderCounter=1:size(rootFolderList,1)
    ...
    commonFolders.folderIn = NeuroWide.utilities.returnFolderStructure(commonFolders.root,4);
    commonFolders.folderList = NeuroWide.utilities.findFoldersMaster(commonFolders.folderIn,{'onephoton','daq','behaviorCam','processed','ephys'});
    ...
    for folderCounter=find([dataIn.goodRun])
        ...
        tmpRaw=AndorDATImporter(dataIn(folderCounter).solis.folder,tmpSettings);
    ...
    for folderCounter=find([dataIn.goodRun])
        ...
        settings.hd=defineHemodynamicParameters(525,625);
        ...
        tmpSettings.frames = dataIn(folderCounter).settings.nframes;
        rawImageChannel=andorDATImporter(dataIn(folderCounter).solis.folder,tmpSettings);
        ...
        if ~isempty(settings.hdChannel1)
            ...
            [Hb,HbO] = NeuroWide.process.calcHb(hdChannel1,hdChannel2);
            clearvars tmp* i*
        ...
        if ~isempty(settings.gfpChannel)
            ...
            [gfp_norm,gfp_norm_HD] = NeuroWide.process.HD_470(gfpChannel,HbO,Hb);
        ...
        if ~isempty(settings.rfpChannel)
            ...
            [rfp_norm,rfp_norm_HD] = NeuroWide.process.HD_565(rfpChannel,hdChannel1,hdChannel2);
...
function processed_signal = process_video(signal,smooth)
    ...
    if smooth
        processed_signal = NeuroWide.process.smooth2d(signal,smooth);
    end
end

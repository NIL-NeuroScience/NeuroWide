function [folders,files] = returnFolderStructure(folderIn,level)
...
%23-05-30 added onephotonSona and onephotonZyla to the list for 1photon
%folder to make distinction afterwards
[tmpFolders,tmpFiles]=NeuroWide.utilities.getFolderContent(folderIn,'*');
...
if ~isempty(tmpFolders)
    for iFolder=1:size(tmpFolders,2)
        ...
        elseif any(strcmpi(folders(iFolder).name,{'1photon','singlephoton','micromanager','solis','onephoton','onephotonSona','onephotonZyla'}))
        ...
        %adds recursion
        if level>1
            [folders(iFolder).folders,folders(iFolder).files]=returnFolderStructure([folders(iFolder).parentFolder,filesep,folders(iFolder).name],level-1);
        end
    end
else
    folders=[];

function f = overview(Signals, GRAB)
...
for i = 1:4
    ax(i) = nexttile([2,1]);
    hold on;
    plot(Signals.t, all_plot(:,i), Color=NeuroWide.colors.r, LineWidth=pS.lW);
    plot(Signals.t, all_plot(:,4+i), Color=NeuroWide.colors.g, LineWidth=pS.lW);
    plot(Signals.t, all_plot(:,8+i), Color=NeuroWide.colors.b, LineWidth=pS.lW);
    if i == 1
...
if isfield(Signals, 'pupil') && ~isempty(Signals.pupil)
    plot(Signals.t, Signals.pupil * 0.4 + 0.6, color=NeuroWide.colors.p, LineWidth=pS.lW);
    plot([0,0], [0.6,1], '-k', lineWidth=3);
end
if isfield(Signals, 'whisker_pad') && ~isempty(Signals.whisker_pad)
    plot(Signals.t, rescale(Signals.whisker_pad, 0.3, 0.6), color=NeuroWide.colors.c, LineWidth=pS.lW);
end

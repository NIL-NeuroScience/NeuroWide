function [Image_Parcellation] = TransformParcellation(parcellation,image,transform,isLeft)
...
if isLeft == 2
    ...
    for x = 1:length(LeftROIs)
        temp = NeuroWide.allenAtlas.ApplyRegistration(cell2mat(LeftROIs(x)),image,InverseTransform);
    ...
    for x = 1:length(RightROIs)
        temp = NeuroWide.allenAtlas.ApplyRegistration(cell2mat(RightROIs(x)),image,InverseTransform);
        temp(find(temp>=0.5)) = 1;
        temp(find(temp<0.5)) = 0;

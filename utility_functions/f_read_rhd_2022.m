function out=f_read_rhd_2022(filename)
...
num_amplifier_samples = int32(num_samples_per_data_block * num_data_blocks);
num_aux_input_samples = int32((num_samples_per_data_block / 4) * num_data_blocks);
num_supply_voltage_samples = int32(1 * num_data_blocks);
num_board_adc_samples = int32(num_samples_per_data_block * num_data_blocks);
num_board_dig_in_samples = int32(num_samples_per_data_block * num_data_blocks);
num_board_dig_out_samples = int32(num_samples_per_data_block * num_data_blocks);
...
if (data_present)
    ...
    for i=1:num_board_dig_in_channels
       ...
       out.board_dig_in_data(i, :) = (bitand(board_dig_in_raw, mask) > 0);
    end
    for i=1:num_board_dig_out_channels
       mask = 2^(board_dig_out_channels(i).native_order) * ones(size(board_dig_out_raw));
       out.board_dig_out_data(i, :) = (bitand(board_dig_out_raw, mask) > 0);
    end

    out.amplifier_channels=amplifier_channels;
    % Scale voltage levels appropriately.
    out.amplifier_data = 0.195 * (amplifier_data - 32768); % units = microvolts
    out.aux_input_data = 37.4e-6 * aux_input_data; % units = volts
    out.supply_voltage_data = 74.8e-6 * supply_voltage_data; % units = volts
    if (board_mode == 1)
        out.board_adc_data = 152.59e-6 * (board_adc_data - 32768); % units = volts
    elseif (board_mode == 13) % Intan Recording Controller
        out.board_adc_data = 312.5e-6 * (board_adc_data - 32768); % units = volts    
    else
        out.board_adc_data = 50.354e-6 * board_adc_data; % units = volts
    end
    out.temp_sensor_data = temp_sensor_data / 100; % units = deg C
    ...
    out.t_amplifier = t_amplifier / sample_rate;
    out.t_aux_input = t_amplifier(1:4:end);
    out.t_supply_voltage = t_amplifier(1:num_samples_per_data_block:end);
    out.t_board_adc = t_amplifier;
    out.t_dig = t_amplifier;
%    out.t_temp_sensor = t_supply_voltage;
    ...
    if (notch_filter_frequency > 0 && data_file_main_version_number < 3)
        ...
        for i=1:num_amplifier_channels
            out.amplifier_data(i,:) = ...
                notch_filter(out.amplifier_data(i,:), sample_rate, notch_filter_frequency, 10);

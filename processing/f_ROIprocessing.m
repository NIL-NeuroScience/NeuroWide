function f_ROIprocessing(varargin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% signals.
% 
% INPUTS: f_ROIprocessing_WF(Mouse,Date,Runs,_)
%   Mouse: mouse ID
%   Date: session date
...
ROI_exist = f_checkFile_WF(fullfile(files.save_dir,'ROIs.mat'));

dataIn_idx = [dataIn.runnum]==list_runs(1);

if p.Results.newROIs || ~ROI_exist
    [brain_mask,hem] = f_createROIs_WF(dataIn(dataIn_idx).template(:,:,1));
    ...
    while ~vessel_good
        vessel_mask = f_maskVessel_WF(dataIn(dataIn_idx).template(:,:,idx_525),sensitivity,brain_mask);
        imagesc(dataIn(dataIn_idx).template(:,:,idx_525));
        clim([0,65535]);
    ...
    save(fullfile(files.save_dir,'ROIs.mat'),'brain_mask','vessel_mask');

    if p.Results.behCam
        cam_folder = fullfile(files.camera,sprintf('Run%02i',list_runs(1)));
        cam_images = natsortfiles(dir(cam_folder));
        imgPath = fullfile(cam_folder,cam_images(round((numel(cam_images)-2)/2+2)).name);
        behaviorROIs = f_drawBehaviorROIs_WF(imgPath);
...
if p.Results.newROIs || ~allen_exist
    ...
    rawData = f_AndorDATImporter_WF(fullfile(files.load_dir,'onephoton',sprintf('Run%02i',list_runs(1))),importSettings);
    ...
    axis image off;
    colormap cmpinf;
    clim(prctile(rawData(:),[0.1,99.9]));
    rotation = input('Rotation? (counter-clockwise)');
    close(f);
    
    parcellation = f_AllenAtlas_WF(imrotate(rawData,rotation),brain_mask,hem);
...
for Run = list_runs
    ...
    if gfp_exist
        ...
        Signals.gfp_HD = f_parcellate_WF(gfp_HD,masks.*vessel_mask);
        Signals.gfp = f_parcellate_WF(gfp,masks.*vessel_mask);
    end
    if rfp_exist
        rfp_HD = 100*h5read(files.h5,'/rfp/normHD');
        rfp = 100*h5read(files.h5,'/rfp/norm');
        Signals.rfp_HD = f_parcellate_WF(rfp_HD,masks.*vessel_mask);
        Signals.rfp = f_parcellate_WF(rfp,masks.*vessel_mask);
    end
    HbO = 1e6*h5read(files.h5,'/hemodynamics/HbO');
    HbR = 1e6*h5read(files.h5,'/hemodynamics/Hb');
    HbT = HbO+HbR;
    Signals.HbO = f_parcellate_WF(HbO,masks);
    Signals.HbR = f_parcellate_WF(HbR,masks);
    Signals.HbT = f_parcellate_WF(HbT,masks);

    %% process behavior

    Signals.Acc = f_alignAcc_WF(digitalInput.SonaTrigger,digitalInput.Accelerometer);
    ...
    % behavior extraction
        ...
        cam_images = natsortfiles(dir(cam_folder));cam_images(1:2) = [];
    ...
    try
        f = f_plotOverview_WF(Signals,p.Results.GRAB);
        ...
        for i = 1:run_bins
            iR_metadata = struct;
            if gfp_exist
                [iR_metadata.rMaps.gfpHbT,f] = f_corr_WF(brain_mask.*gfp_HD(:,:,run_idx{i}),HbT(:,:,run_idx{i}),3,plot=true);clim([-1 1]);axis image off;
                title('GFP vs. HbT');
            ...
            if rfp_exist
                if gfp_exist
                    [iR_metadata.rMaps.rfpgfp,f] = f_corr_WF(brain_mask.*gfp_HD(:,:,run_idx{i}),rfp_HD(:,:,run_idx{i}),3,plot=true);clim([-1 1]);axis image off;
                    title('GFP vs. RFP');
                    img_name = sprintf('gfprfpCorr_Run%02i_iRun%02i',Run,i);
                    exportgraphics(f,fullfile(files.images,[img_name '.png']),'Resolution',300,'BackgroundColor',[1 1 1]);
                    savefig(f,fullfile(files.images,[img_name '.fig'])); close(f);
                end
                [iR_metadata.rMaps.rfpHbT,f] = f_corr_WF(brain_mask.*rfp_HD(:,:,run_idx{i}),HbT(:,:,run_idx{i}),3,plot=true);clim([-1 1]);axis image off;
                title('RFP vs. HbT');
